...
bool UnitTest::HeapManager_UnitTest()
{
	...
	customMalloc::MemoryManager * pHeapManager = CreateHeapManager(pHeapMemory, sizeHeap, numDescriptors);
	assert(pHeapManager);
	...
	// an allocation worked. Also helped test my ShowFreeBlocks() and ShowOutstandingAllocations().
	{
		...
		void * pPtr = alloc(pHeapManager, largestBeforeAlloc - pHeapManager->get_minimum_to_leave());
	...
	if (!AllocatedAddresses.empty())
	{
		...
		//std::cout << "size? " << sizeHeap / 2 <<"  "<< pHeapManager->totalFreeMemory << std::endl;
